/* MPU9250_MS5637_BasicAHRS2_Ladybug Basic Example Code
 by: Kris Winer
 date: April 1, 2014
 ...
 Mahony filter algorithms. Sketch runs on the 3.3 V 80 MHz Ladybug STM32L432 Development Board.
 ...
 Note: The MPU9250 is an I2C sensor and uses the Arduino Wire library. It is not 5 V tolerant.
 */
#include "Wire.h"   
#include <SPI.h>
...
void setup()
{
  ...
  Wire.begin(TWI_PINS_20_21); // set master mode, I2C frequency at 400 kHz  
...
// Start device display with ID of sensor
  ...
  display.setCursor(0,60); display.print("Sharp Display");
  display.refresh();
  delay(1000);
...
// Second order corrections
 ...
 //
    ...
    Serial.print("Altitude = "); Serial.print(altitude, 2); Serial.println(" feet");
      ...
      display.setCursor(0,40); display.print("G "); display.print(gx, 1); display.print(" "); display.print(gy, 1); display.print(" "); display.print(gz, 1); display.print(" dps");
      display.setCursor(0,60); display.print("M "); display.print((int) mx); display.print(" "); display.print((int) my); display.print(" "); display.print((int) mz); display.print(" mG");  
      display.refresh();
      }
    ...
    digitalWrite(myLed, 1); delay(10); digitalWrite(myLed, 0); // active high
    count = millis(); 
    sumCount = 0;
    sum = 0;    
    }
...
{  
 ...
 // With the MPU9250, it is possible to get gyro sample rates of 32 kHz (!), 8 kHz, or 1 kHz
  writeByte(MPU9250_ADDRESS, CONFIG, 0x03);  // 41 Hz bandwidth, 5.9 ms delay
 // writeByte(MPU9250_ADDRESS, CONFIG, 0x02);  // 92 Hz bandwidth, 3.9 ms delay

 // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
  writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x05);  // Use a 167 Hz rate; a rate consistent with the filter update rate 
                                    // determined in set in CONFIG above
